...
**`bmad-code-review` 3-layer pass run 2026-07-07 (fresh reviewer, claude-fable-5).** Blind Hunter + Edge Case Hunter + Acceptance Auditor against commit `bc3e3a9` diff (src/, tests/, docs/output-schema-mode1.json), audited against all 9 ACs + Dev Notes constraints. **Acceptance Auditor: 9/9 ACs satisfied, all Dev Notes constraints (transitional strategy, naming split, dropped R-HF-10 keys absent, slim shapes exact-match `PromptBuilder.cs`, do-not-touch list respected) verified — 0 violations.** 18 raw findings across the layers → deduplicated/merged → **0 decision-needed / 4 patch / 5 defer / 6 dismiss**. Patches applied 2026-07-07; build 0 warnings / 0 errors; test suite **316 → 319** green (3 new tests). Notable review event: patch P1 (Lazy `PublicationOnly` so a schema-load exception is not cached for the process lifetime) was applied, **empirically falsified** — 14 test failures with `Json.Schema.JsonSchemaException: Overwriting registered schemas is not permitted`, proving the `$id` registration is process-global and duplicate `FromFile` parses collide — and reverted; `ExecutionAndPublication` is load-bearing and is now documented as such in the code, with the cached-exception cost reclassified to defer.

- [x] [Review][Patch] P2 — JSON `null` literal bypassed the schema gate: `JsonNode.Parse("null")` returns a null reference which the guard conflated with a parse failure, silently skipping validation (surfaced later as `AI_ERROR` instead of `OUTPUT_SCHEMA_VIOLATION`). Fixed with a `parsedAsJson` flag so a parse *success* always validates (the validator rejects a null root as `type != object`); pinned by `Validate_NullInstance_FailsWithTypeViolation` + `ProcessAgentResponse_NullLiteralResponse_ThrowsOutputSchemaViolation` [src/DataverseDocAgent.Api/Features/DocumentGenerate/DocumentGenerateService.cs:221-241] (blind+edge)
- [x] [Review][Patch] P3 — NFR-007 pipeline leak test was tautological (asserted a hardcoded-constant exception message contains no secrets; never inspected the actual logged detail). Added `ProcessAgentResponse_SchemaViolation_LoggedPathsContainNoInstanceValues` with a capturing `ILogger` spy: positive anchor on the schema-validation warning being logged, then asserts sentinel instance values never appear in it [tests/DataverseDocAgent.Tests/DocumentGenerateServiceTests.cs] (blind)
- [x] [Review][Patch] P4 — validator comment claimed pointers "never contain instance values" without acknowledging they DO contain instance property NAMES (hallucinated root keys on `additionalProperties` failures; table-logicalName map keys). Comment sharpened: names are structural/public-metadata, never credential material — the precise NFR-007 claim [src/DataverseDocAgent.Api/Agent/OutputSchemaValidator.cs FormatLocation] (blind)
- [x] [Review][Patch] P1 — attempted `LazyThreadSafetyMode.PublicationOnly` (avoid caching a schema-load exception forever); **reverted after empirical falsification** (global `$id` registry collision under parallel duplicate `FromFile`, 14 test failures). Landed as a load-bearing-mode comment on the ctor; cached-exception cost moved to defer [src/DataverseDocAgent.Api/Agent/OutputSchemaValidator.cs ctor] (blind+edge)
- [x] [Review][Defer] Cached schema-load exception + misclassification — a missing/corrupt schema file at runtime throws from `Lazy.Value`, is cached for the process lifetime (`ExecutionAndPublication`), and surfaces via the generic catch as `AI_ERROR "Agent orchestration failed"` with `safeToRetry:true` inviting futile retries. `PublicationOnly` proven not viable; the honest fix is eager load-or-fail at startup or a catch-and-wrap config-fault code. Resurface: Story 4.12 orchestrator restructure, any `JsonSchema.Net` upgrade, or first deployment incident [src/DataverseDocAgent.Api/Agent/OutputSchemaValidator.cs] (blind+edge)
- [x] [Review][Defer] Process-global `SchemaRegistry` `$id` collision when a second validator instance/container evaluates — **empirically confirmed during this review** (the P1 experiment). Latent landmine for any future `WebApplicationFactory` integration test or second DI container; tests currently mitigate via the single `SharedValidator`. Resurface: adding any integration test that boots the host, or `JsonSchema.Net` upgrade [src/DataverseDocAgent.Api/Agent/OutputSchemaValidator.cs + Program.cs DI] (blind+edge)
- [x] [Review][Defer] Transitional item defs accept `{}` — `relationshipItem` and `applicationUserItem` have no `required` array, so an empty object validates as a relationship/app-user (matches the lenient shipped deserializer; tightening risks false-positive gate rejections of live output). Resurface: Story 4.9/4.11 schema version bump [docs/output-schema-mode1.json] (blind)
- [x] [Review][Defer] `publisherPrefixSummary` is a fully loose `{"type":"object"}` stub — arbitrary nested content escapes the strict gate inside that optional subtree. Documented intentional v1.0.0 placeholder. Resurface: Story 4.2 (deterministic analyser) pins the shape via version bump [docs/output-schema-mode1.json] (edge+auditor)
- [x] [Review][Defer] Schema-load failure surfaces unclassified (generic `AI_ERROR` bucket rather than a config-fault code) — folded into the cached-exception defer above; same resurface triggers [src/DataverseDocAgent.Api/Features/DocumentGenerate/DocumentGenerateService.cs catch chain] (blind)
- [x] [Review][Dismiss] Double-parse of the payload (gate parses `JsonNode`, `ParseAgentJson` re-parses) — intentional defence-in-depth per Dev Notes; one extra parse per job is negligible (blind) — dismissed
- [x] [Review][Dismiss] "Validated node may not equal deserialized node" — verified false: both paths apply the identical pure `TrimToJsonObject(StripCodeFences(...))` to the same input, and `s_jsonOptions` adds no leniency that could diverge from `JsonNode.Parse` (blind, refuted by edge) — dismissed
- [x] [Review][Dismiss] `applicationUsers` vs `app_users` naming duality — spec-mandated: transitional required key + AC-2 target section coexist by design until the 4.9/4.11 version bump (blind) — dismissed
- [x] [Review][Dismiss] Unknown-key test asserts value absence but not key absence — key names in pointers are by design (spec mandates `(instanceLocation, evaluationPath/keyword)` pairs; pointers inherently contain keys), so a key-absence assertion would contradict the contract (blind) — dismissed
- [x] [Review][Dismiss] `null!` for `connectionFactory`/`orchestratorFactory` in `BuildServiceWithSpyStore` — house delegate-seam style; a refactor touching those deps fails noisily in-test (blind) — dismissed
- [x] [Review][Dismiss] Gate-vs-deserializer case-sensitivity divergence (schema is case-sensitive, STJ deserializer is not) — the intended tightening: PascalCase drift now fails loudly as `OUTPUT_SCHEMA_VIOLATION` instead of being silently half-parsed (edge) — dismissed
